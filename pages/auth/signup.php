...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$terms) {
        ...
        try {
            ...
            // 전화번호 정규화 및 중복 체크 (이메일 대신)
            if ($phone !== '') {
                // normalize_phone 함수가 있으면 사용, 없으면 직접 정규화
                if (function_exists('normalize_phone')) {
                    $normalizedPhone = normalize_phone($phone);
                } else {
                    $normalizedPhone = preg_replace('/[^0-9]/', '', $phone);
                }
                $st = $pdo->prepare('SELECT 1 FROM users WHERE phone = ?');
            ...
            if (!$error) {
                ...
                // 이메일은 UUID 기반 더미 이메일로 생성 (DB NOT NULL 제약 대응)
                // UUID를 사용하여 항상 고유한 이메일 생성
                $dummyEmail = 'user_' . str_replace('-', '', $id) . '@dolbom.local';
                
                // 더미 이메일 중복 체크 (거의 발생하지 않지만 안전을 위해)
                $st = $pdo->prepare('SELECT 1 FROM users WHERE email = ?');
                $st->execute([$dummyEmail]);
                if ($st->fetch()) {
                    // 중복되면 다시 UUID 생성 (극히 드문 경우)
                    $id = uuid4();
                    $dummyEmail = 'user_' . str_replace('-', '', $id) . '@dolbom.local';
                }
                ...
                $st->execute([
                    $id,
                    $dummyEmail, // 더미 이메일 (DB NOT NULL 제약 대응)
                    $hash,
                    $name,
                    ($phone !== '' ? (function_exists('normalize_phone') ? normalize_phone($phone) : preg_replace('/[^0-9]/', '', $phone)) : null),
...
<div class="mx-auto max-w-md mt-48 text-base">
    ...
    <p class="mt-2 text-lg text-gray-600">정보를 입력하세요.</p>
    <?php if ($error): ?>
