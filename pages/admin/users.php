...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        <form method="get" class="flex gap-2">
            ...
            <?php if ($search): ?>
            <a href="<?= $base ?>/admin/users" class="min-h-[44px] px-6 border border-gray-300 rounded-lg hover:bg-gray-50 inline-flex items-center">초기화</a>
            <?php endif; ?>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">마지막 로그인</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900"><?= htmlspecialchars($user['name']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-900"><?= htmlspecialchars($user['email']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-900"><?= htmlspecialchars($user['phone'] ?? '-') ?></td>
                        <td class="px-4 py-3 text-sm text-gray-900"><?= htmlspecialchars($user['created_at']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-900"><?= htmlspecialchars($user['last_login_at'] ?? '-') ?></td>
                    ...
                    <?php else: ?>
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-gray-500">회원이 없습니다.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
